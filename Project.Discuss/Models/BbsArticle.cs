...
using Project.Discuss.Models.Coins;
using System;
...
public partial class BbsArticle
{
    ...
    // ImageAddress 存储原始的 JSON 字符串
    public string? ImageAddress { get; set; }

    // 存储反序列化后的 ImgDto 数据
    [NotMapped]
    public List<ImgDto>? ImageAddressDto
    {
        get
        {
            // 如果 ImageAddress 不为空，反序列化它
            if (!string.IsNullOrEmpty(ImageAddress))
            {
                var options = new JsonSerializerOptions
                {
                    PropertyNamingPolicy = JsonNamingPolicy.CamelCase // 忽略大小写
                };
                return JsonSerializer.Deserialize<List<ImgDto>>(ImageAddress,options);
            }
            return null;
        }
        set
        {
            // 如果要设置 ImageAddressDto，序列化它
            if (value != null)
            {
                var options = new JsonSerializerOptions
                {
                ...
                ImageAddress = JsonSerializer.Serialize(value,options);
            }
            else
            {
                ImageAddress = null;
...
public short? StandPoint { get; set; }
    ...
    public virtual BbsUserinfo User { get; set; }
  
    public virtual BbsArticleContent BbsArticleContent { get; set; }
}
